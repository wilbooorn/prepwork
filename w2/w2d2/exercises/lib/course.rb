...
class Course
  ...
  def initialize(name, dept, credits, days = [], time = nil)
    @name = name
    @department = dept
    @credits = credits
    @students = []
  ...
  def conflicts_with?(course)
    self.days.each do |self_day|
      course.days.each do |course_day|
        return true if (self.time_block == course.time_block) && (self_day == course_day)
      end
    end
    false
  end
